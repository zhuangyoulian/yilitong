...
{
    $time = gmtime();
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand,b.brand_logo, m.type_money AS bonus_money, wms.name AS wms_name, cc.cn_name, ' .
            ...
            " LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            " LEFT JOIN " . $GLOBALS['ecs']->table('wms') . " AS wms ".
                "ON wms.id = g.default_warehouse ".
            " LEFT JOIN " . $GLOBALS['ecs']->table('country') . " AS cc ".
                "ON cc.country_id = g.country ".
    ...
    if ($row !== false)
    {
        ...
        $row['shop_price_formated'] = number_format($row['shop_price'], 2, '.', '');
        ...
        $row['promote_price'] =  number_format($promote_price, 2, '.', '');
...
function get_goods_gallery($goods_id)
{
	...
	$sql = 'SELECT img_id, img_url, thumb_url, img_desc' .
			' FROM ' . $GLOBALS['ecs']->table('goods_gallery') .
			" WHERE goods_id = '$goods_id' order by img_desc ";
...
function get_products($goods_id){
	$sql = "SELECT * FROM " .$GLOBALS['ecs']->table('products'). " WHERE goods_id = '$goods_id'";
	return $GLOBALS['db']->getAll($sql);
}

function get_goods_spec_list($goods_id){
	$sql = "SELECT g.*,s.spec_name FROM " .$GLOBALS['ecs']->table('goods_spec_index'). "g inner join ".$GLOBALS['ecs']->table('specification')."s on g.spec_id=s.spec_id WHERE goods_id = '$goods_id'";
	$rs = $GLOBALS['db']->getAll($sql);
	$sv = array();
	foreach ($rs as $v){
		$sv[$v['spec_name']][] = $v['spec_value_id'];
	}
	$newsv = array();
	foreach ($sv as $key=>$v){
		$newsv[$key] = array_unique($v);
	}
	
	return $newsv;
}

function get_spac_value($value_id){
	$sql = "select spec_value from ".$GLOBALS['ecs']->table('spec_values')." where spec_value_id=".$value_id;

...
function get_pager($url, $param, $record_count, $page = 1, $size = 10)
{
    ...
    else
    {
        ...
        $pager['page_last']  = ($_to < $page_count) ? $url_format . $page_count : '';
        $pager['page_kbd']  = ($_pagenum < $page_count) ? true : false;
        if(empty( $pager['page_first'] )){
        	$pager['page_first']   = $url . $param_url . 'page=1';
        }
        if(empty($pager['page_last'])){
        	$pager['page_last']    = $url . $param_url . 'page=' . $page_count;
        }
        $pager['page_number'] = array();
        for ($i=$_from;$i<=$_to;++$i)
...
function _show_msg()
{
	...
	//$GLOBALS['smarty']->assign('auto_redirect', $auto_redirect);
	//$GLOBALS['smarty']->assign('message', $msg);
	$GLOBALS['smarty']->display('mobile_create_order_mess.html');
...
function logwrite($word='') {
	$fp = fopen("temp/paybacklog.txt","a");
	flock($fp, LOCK_EX) ;
...
function logapi($word='',$type=0) {
	$file = "";
	$y=date('Y', time());
	$m=date('m', time());
	$dir=ROOT_PATH."temp/querylog";
	...
	if($type==1){
		//$file = "/temp/android_api.txt";
		$file=$file."/{$y}_{$m}_android_api.txt";
	}
	if($type==2){
		//$file = "/temp/ios_api.txt";
		$file="/{$y}_{$m}_ios_api.txt";
	}
	if($type==3){
		//$file = "/temp/wms_api.txt";
		$file="/{$y}_{$m}_wms_api.txt";
	}
	$fp = fopen($dir .$file,"a");
	flock($fp, LOCK_EX) ;
	fwrite($fp,"执行日期：".strftime("%Y%m%d%H%M%S",time())."\n".$word."\n");
...
function handle_order($user_id=''){
	$sql = "update ".$GLOBALS['ecs']->table('order_info'). " set order_status=5 where order_status=0 and unix_timestamp( now( ) )>limit_time ";
	if(empty($user_id)){
...
//  区分总、子订单  liujiyuan   2016-3-17
function handle_order_is_main($user_id=''){
    $sql = "SELECT order_id FROM " .$GLOBALS['ecs']->table('order_info'). " WHERE is_main = 1 AND user_id = '" .$user_id. "' ORDER BY order_id DESC LIMIT 1";
    $last_order_id = $GLOBALS['db']->getOne($sql);

    if(empty($last_order_id)){
        $last_order_id = 0;
    }

    $sql = "SELECT distinct(parent_id) FROM " .$GLOBALS['ecs']->table('order_info'). " WHERE parent_id > 0 AND user_id = '" .$user_id. "' AND order_id > " .$last_order_id;
    $order_id_arr = $GLOBALS['db']->getAll($sql);

    if(count($order_id_arr)>0){
        $order_id_str = '';
        foreach($order_id_arr as $k => $v){
            $order_id_str .= $v['parent_id'].',';
        }
        $order_id_str = rtrim($order_id_str, ',');
        $sql = "UPDATE " .$GLOBALS['ecs']->table('order_info'). " SET is_main = 1 WHERE order_id IN($order_id_str) AND user_id = '" .$user_id. "'";
        $GLOBALS['db']->query($sql);
...
//推送日志记录
function push_logs($userid,$message,$type,$message_type='',$order_sn='',$invert_id=''){
 if($userid && $message && $type){	
  $temptime=time();
  $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('users_push_log') . ' ( ' .
                'user_id, type, remark, addtime, message_type, order_sn, invited_id ' .') VALUES (' .
                "'$userid','$type', '{$message}', '{$temptime}', '{$message_type}', '{$order_sn}', '{$invert_id}')";
    $GLOBALS['db']->query($sql);
 }
...
//推送消息记录条数
function user_push($userid,$type){
	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('users_push') . " WHERE user_id ={$userid} AND app_type=0";
	$users_push = $GLOBALS['db']->getRow($sql);
	if($users_push){
	if($type=='1'){
		$sql = "UPDATE " .$GLOBALS['ecs']->table('users_push'). " SET".
		" team_num = team_num + 1 , teams_num = teams_num + 1 ".
		" WHERE user_id = '" . $userid . "' AND app_type=0 ";
		$GLOBALS['db']->query($sql);
	}elseif($type=='2'){
		$sql = "UPDATE " .$GLOBALS['ecs']->table('users_push'). " SET".
		" order_num = order_num + 1 , wealth_num = wealth_num + 1 ".
		" WHERE user_id = '" . $userid . "' AND app_type=0";
		$GLOBALS['db']->query($sql);
	}
	}else{
		$temptime=time();
		//type=1   团队信息记录条数，type=2 订单记录条数
		if($type=='1'){
		$sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('users_push') . ' ( ' .
				'user_id, team_num,teams_num, addtime ' .') VALUES (' .
						"'$userid','1','1','{$temptime}')";
		$GLOBALS['db']->query($sql);
		}else if($type=='2'){
		$sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('users_push') . ' ( ' .
				'user_id, order_num, wealth_num, addtime ' .') VALUES (' .
						"'$userid','1','1','{$temptime}')";
		$GLOBALS['db']->query($sql);
		}
	} 

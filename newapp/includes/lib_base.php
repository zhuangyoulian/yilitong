...
function postmail($to,$subject = '',$body = ''){
	...
	include_once(ROOT_PATH . 'includes/class.smtp.php');
	...
	$mail->Username   = 'ses@youkastation.com';  // SMTP服务器用户名，PS：我乱打的
	$mail->Password   = 'WWWb2c0755';            // SMTP服务器密码
	$mail->SetFrom('ses@youkastation.com', 'ses');
	$mail->AddReplyTo('ses@youkastation.com','ses');
...
//检查实名
function check_real_name($realname){
	$flag=preg_match("/[\x7f-\xff]/", $realname);
	$result=array('rs'=>1,'info'=>'验证成功');
	if($flag==false){
		$result['rs']=0;
		$result['info']="请输入真实姓名";//只能为中文汉字
		return $result;
	}
	$lenA= str_len($realname); //检测字符串实际长度
	$lenB= mb_strlen($realname, "utf-8");
	if($lenA%$lenB!=0){
		$result['rs']=0;
		$result['info']="请输入真实姓名";
		return $result;
	}
	$strlen=str_len($realname);
	if($strlen<4 || $strlen>30){
		$result['rs']=0;
		$result['info']="请输入真实姓名";//字数只能为2-15个
		return $result;
	}

	$arr = array('先生','小姐','太太','男士','女士');
	foreach ($arr as $val){
		if(strpos('%**#'.$realname,$val)>0){
			$result['rs']=0;
			$result['info']="请输入真实姓名";//不能填写先生、小姐、太太、男士、女士
			return $result;
		}
	}

	$str=mb_strimwidth($realname,0,2,'','utf8');
	if($str=='小'){
		$result['rs']=0;
		$result['info']="请输入真实姓名"; //第一个字不能是小字
		return $result;
	}
	return $result;
}

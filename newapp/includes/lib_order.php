...
/**
 * [get_parent_order 获取主单号]
 * @Author   [zihang]
 * @DateTime [2016-02-26T16:09:37+0800]
 * @param    [type]                     $order_id [主单id]
 * @return   [type]                               [description]
 */
function get_parent_order($order_id){
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
        ...
        // $order['formated_add_time']       = local_date(, $order['add_time']);
...
function order_fee($order, $goods, $consignee)
{
    ...
    /* if($total['goods_price']<99){
    	$total['shipping_fee'] = 15;
    } */
...
function newaddto_cart($goods_id, $num = 1, $product_id,$spec = array(), $parent = 0)
{
	$GLOBALS['err']->clean();
	$_parent_id = $parent;
	...
	$sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
			"g.market_price, g.shop_price AS org_price, g.promote_price, g.promote_start_date, ".
			"g.promote_end_date, g.goods_weight, g.integral, g.extension_code, ".
			"g.goods_number, g.is_alone_sale, g.tax_number,g.default_warehouse,g.is_shipping,".
			"IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price ".
			" FROM " .$GLOBALS['ecs']->table('goods'). " AS g ".
			" LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
			"ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
	...
	//检查product_id对应的库存(选仓库，默认w1)
	if ($num > $product_info['w'.$goods['default_warehouse']])
	{
		$GLOBALS['err']->add('库存没货!');
		//可邮件短信提醒
    ...
    if ($product_info['product_id']){
        $sql = "SELECT product_sn FROM " .$GLOBALS['ecs']->table('products').
            " WHERE product_id = " .$product_info['product_id'];
        $product_sn = $GLOBALS['db']->getOne($sql);
        $parent['goods_sn'] = $product_sn;
    }
 ...
 */
	...
	//{
		...
		if($row) //如果购物车已经有此物品，则更新
		{
			...
			//再次检查库存
			if($num > $product_info['w'.$goods['default_warehouse']]){
				$GLOBALS['err']->add('库存不足!');
				//可邮件短信提醒
				
				return false;
			}
			
			$price=get_remote_price($goods_id);
			if($price){
				$goods_price=$price;
			}else{
			  $goods_price = get_final_price($goods_id, $num, true, $spec);
			...
			//检查关税
			//$rate_price = get_taxrate($product_info['tax_number'])*$goods_price;
...
/****清除提交生成订单的商品***/
function clear_cart_ids($goods_ids,$type = CART_GENERAL_GOODS){
	$goods_ids=trim($goods_ids,',');
	 $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') .
            " WHERE goods_id in ({$goods_ids}) AND user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '$type'";
    $GLOBALS['db']->query($sql);
...
function get_cart_goods()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {  
    	$price=get_remote_price($row['goods_id']);
    	$row['goods_price']=empty($price) ? $row['goods_price'] : $price;
    	$row['subtotal_notf']     = $row['goods_price'] * $row['goods_number'];
    	$row['goods_price_notf']  = $row['goods_price'];
    	$row['market_price_notf'] = $row['market_price'];
    	
    	$total['goods_price']  += $row['goods_price'] * $row['goods_number'];
    	$total['market_price'] += $row['market_price'] * $row['goods_number'];
    	
    	
        $tax_rate = get_taxrate_by_goods_id($row['goods_id']);
        if(empty($tax_rate)){
        	$tax_rate = 0;
        }
        $total['goods_tax_amount'] += $tax_rate* $row['goods_price'];
        $gt = get_weigh_by_goods_id($row['goods_id']);
        $weight = $gt['goods_weight'];
        $total['goods_weight'] += $weight* $row['goods_number'];
        ...
        $row['weight'] = $weight* $row['goods_number'];
        
        $row['default_warehouse'] = $gt['default_warehouse'];
        $row['take_byself'] = $gt['take_byself'];
        $row['tax_rate'] = $tax_rate;
        $row['unit_tax'] = $tax_rate*$row['goods_price_notf'];
        
        $row['spec'] = "";//规格
        $row['is_on_sale']=get_goods_sale($row['goods_id']);
        /* 统计实体商品和虚拟商品的个数 */
        if ($row['is_real'])
        {
            $real_goods_count++;
        ...
        $sql = "select spec_desc from ".$GLOBALS['ecs']->table('products')." where product_id=".$row['product_id'];
        $spec = $GLOBALS['db']->getOne($sql);
        if(!empty($spec)){
        	$arr_spec_desc = unserialize($spec);
        	$row['spec'] = implode(",", $arr_spec_desc['spec_value']);
        }

        /* 商品对应仓库的库存数   liujiyuan   2015-12-29 */
        if ($row['default_warehouse'] > 0){
            $select_stock = 'w'.$row['default_warehouse'];
            $sql = "SELECT $select_stock FROM " .$GLOBALS['ecs']->table('products'). " WHERE goods_id = '{$row['goods_id']}'
                AND product_id = '{$row['product_id']}'";
            $stock = $GLOBALS['db']->getOne($sql);
            $row['stock'] = empty($stock) ? 0 : $stock;
        }
        //商品无库存，下架。不加商品价格
        if(empty($row['stock']) || empty($row['is_on_sale'])){
        	$total['goods_price']  -= $row['goods_price'] * $row['goods_number'];
        	$total['market_price'] -= $row['market_price'] * $row['goods_number'];
        	$total['remark_num'] +=$row['goods_number'];
        	$is_remark=+1;
        }
        /* 增加是否在购物车里显示商品图 */
...
function get_consignee($user_id)
{
    ...
    else
    {
        ...
        if ($user_id > 0)
        {
        	...
        	$sql = "select ua.address_id,ua.consignee,ua.area,ua.address,ua.mobile,ua.tel,ua.email,u.address_id default_address_id from ". $GLOBALS['ecs']->table('user_address') ." ua left join ".$GLOBALS['ecs']->table('users')." u on ua.user_id=u.user_id where ua.user_id=".$user_id;
 ...
 * 是否取推荐价格
 */
function get_remote_price($goods_id){
	$start1 = local_strtotime( date('Y-m-d 00:00:00') );
	$end1 = local_strtotime( date('Y-m-d 23:59:59') )+1;
	
	$where=" WHERE promote_start_date <=$start1 AND promote_end_date>=$end1 AND is_promote =1 AND promote_price>0 AND goods_id={$goods_id}";
	$sql="SELECT promote_price FROM " .$GLOBALS['ecs']->table('goods'). " $where ";
...
function get_taxrate($taxno){
	$sql = "select ifnull(tax_rate,0) as tax_rate from ". $GLOBALS['ecs']->table('tax_rate') ." where tax_number='$taxno'";
	return $GLOBALS['db']->getOne($sql);
}

//  根据商品ID，取得商品税率   2015-12-25
function get_taxrate_by_goods_id($goods_id){
	$sql = "select ifnull(t.tax_rate,0) as tax_rate from ". $GLOBALS['ecs']->table('tax_rate') ." t inner join ".$GLOBALS['ecs']->table('goods') ." g on g.tax_number=t.tax_number where g.goods_id='$goods_id'";
	return $GLOBALS['db']->getOne($sql);
}

function get_weigh_by_goods_id($goods_id){
	$sql = "select goods_weight,default_warehouse,take_byself from ".$GLOBALS['ecs']->table('goods') ." where goods_id=".$goods_id;
	return $GLOBALS['db']->getRow($sql);
}

function get_goods_sale($goods_id){
	$sql="SELECT is_on_sale FROM " . $GLOBALS['ecs']->table('goods') . " where goods_id={$goods_id}";
 ...
 * @return  array   购物车商品数组
 */
function cart_goods_list($type = CART_GENERAL_GOODS, $rec_id = '')
{
    if (!empty($rec_id)){
        $where = " AND rec_id IN ($rec_id)";
    }
    $sql = "SELECT A.rec_id, A.user_id, A.session_id, A.goods_id, A.goods_sn, A.product_id, A.goods_name, A.market_price, " .
        "A.goods_price, A.goods_number, A.goods_attr, A.is_real, A.extension_code, A.parent_id, A.rec_type, A.is_gift, A.is_shipping, " .
        "A.can_handsel, A.goods_attr_id, A.goods_price * A.goods_number AS subtotal_notf, A.goods_price AS goods_price_notf, " .
        "A.market_price AS market_price_notf, A.market_price * A.goods_number AS market_price_notf, " .
        "B.goods_weight AS unit_weight, B.goods_weight * A.goods_number AS weight, B.default_warehouse,B.is_on_sale, B.take_byself, " .
        "C.tax_rate, C.tax_rate * A.goods_price AS unit_tax, B.goods_thumb,B.commission  " .
        "FROM " .$GLOBALS['ecs']->table('cart'). " AS A LEFT JOIN " .$GLOBALS['ecs']->table('goods'). " AS B ON B.goods_id = A.goods_id " .
        "LEFT JOIN " .$GLOBALS['ecs']->table('tax_rate'). " AS C ON C.tax_number = B.tax_number " .
        " WHERE user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '$type'";
    $sql = $sql . $where;

    $arr = $GLOBALS['db']->getAll($sql);
    foreach ($arr as $k => $v){
        $arr[$k]['spec'] = get_goods_spec($v['product_id']);
        $arr[$k]['stock'] = get_goods_stock($v['product_id'], $v['default_warehouse']);
        $arr[$k]['subtotal'] = '￥'.($v['goods_price']*$v['goods_number']);
        $arr[$k]['market_price'] = '￥'.$v['market_price'];
        $arr[$k]['goods_price'] = '￥'.$v['goods_price'];
        $arr[$k]['unit_tax'] =  sprintf("%.2f", $v['unit_tax']);
        $arr[$k]['unit_weight'] =  sprintf("%.0f", $v['unit_weight']);
        /* if ($v['default_warehouse'] > 0){
        	$select_stock = 'w'.$v['default_warehouse'];
        	$sql = "SELECT $select_stock FROM " .$GLOBALS['ecs']->table('products'). " WHERE goods_id = '{$v['goods_id']}'
        	AND product_id = '{$v['product_id']}'";
        	$stock = $GLOBALS['db']->getOne($sql);
        	$v['stock'] = empty($stock) ? 0 : $stock;
        }
        //删除下架和没有库存商品
        if(empty($v['is_on_sale']) || empty($v['stock'])){
        	unset($arr[$k]);
        } */
...
/* 取商品的对应库存 liujiyuan    2015-12-30 */
function get_goods_stock($product_id, $default_warehouse){
    $stock = 'w' . $default_warehouse;
    $sql = "SELECT $stock FROM " .$GLOBALS['ecs']->table('products'). " where product_id=" .$product_id;
    $spec = $GLOBALS['db']->getOne($sql);

    return $spec;
}

/* 取商品的对应的规格属性 liujiyuan    2015-12-30 */
function get_goods_spec($product_id){
    $sql = "SELECT spec_desc FROM " .$GLOBALS['ecs']->table('products'). " where product_id=" .$product_id;
    $spec = $GLOBALS['db']->getOne($sql);
    if(!empty($spec)){
        $arr_spec_desc = unserialize($spec);
        $spec = implode(",", $arr_spec_desc['spec_value']);
    }
 ...
 * a)   消费税＝售价÷（1-消费税比例税率）×消费税比例税率
 * b)  增值税＝（售价+消费税）×增值税税率
 * c)  商品税金= ( 消费税+增值税)*70%
 * d)  订单税金=订单中所有商品税金之和消费税 
 * @param  string $procuct_id    procuct_id
 * @return 总税
 */
function get_tax($price,$product_id){
    $sql = "SELECT nt.goods_rate,nt.goods_rate_vat FROM ".$GLOBALS['ecs']->table('number_tax')." nt
            inner join ".$GLOBALS['ecs']->table('products')." p 
            on p.product_sn=nt.product_sn
            WHERE p.product_id='".$product_id."'";
    $row = $GLOBALS['db']->getRow($sql);
    //消费税
    $excise = ($price/(1-$row['goods_rate']))*$row['goods_rate'];
    //增值税
    $vat = ($price+$excise)*$row['goods_rate_vat'];
    //总税
    $tax = ($excise+$vat)*0.7;

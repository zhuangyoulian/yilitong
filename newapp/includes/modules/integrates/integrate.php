...
class integrate
{
    ...
    function login($username, $password, $remember = null)
    {
        if ($this->check_user($username, $password) > 0)
        {
            ...
            //多方式登录(用户名,email，手机，最后在取一次username)
            $sql = "SELECT user_name FROM " . $this->table('users').
            " WHERE user_name='$username' or email='$username' or mobile_phone='$username' ";
    ...
    function add_user($username, $password, $email, $gender = -1, $bday = 0, $reg_date=0, $md5password='')
    {
       ...
       /*  $sql = "SELECT " . $this->field_id .
        ...
        if ($md5password)
        {
            $post_password = $this->compile_password(array('md5password'=>$md5password,'user_name'=>$post_username,'reg_time'=>$reg_time));
        }
        else
        {
            $post_password = $this->compile_password(array('password'=>$password,'user_name'=>$post_username,'reg_time'=>$reg_time));
        }

        $fields = array('real_ip', 'real_ip_where', $this->field_name, $this->field_email, $this->field_pass,'reg_time');
        $values = array($_SERVER['REMOTE_ADDR'], '1', $post_username, $email, $post_password,$reg_time);
    ...
    function edit_user($cfg)
    {
        ...
        {
            $values[] = $this->field_pass . "='" . $this->compile_password(array('md5password'=>$cfg['md5password'],'user_name'=>$cfg['post_username'],'reg_time'=>$cfg['reg_time'])) . "'";
        }
       ...
       switch ($cfg['type'])
       {
           case PWD_MD5 :
			   ...
			   else
		       {
                    ...
                    //改写加密方式
		       		return 's'.substr(md5($cfg['md5password'].$cfg['user_name'].$cfg['reg_time']),0,31);
			   }

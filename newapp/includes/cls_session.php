...
class cls_session
{
    ...
    {
    	/* $m_info = explode(":",$this->m_info );
    	$m = new Memcache;
    	$m->addServer($m_info[0], $m_info[1]); 
    	$this->cls_session($m, $session_table, $session_data_table, $session_name, $session_id);
    	*/
    ...
    function cls_session(&$db, $session_table, $session_data_table, $session_name = 'ECS_ID', $session_id = '')
    {
        ...
        else
        {
            ...
            setcookie($this->session_name, $this->session_id . $this->gen_session_key($this->session_id), time()+3600*24*10, $this->session_cookie_path, $this->session_cookie_domain, $this->session_cookie_secure);
        }
    ...
    function load_session()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (!empty($session['data']) && $this->_time - $session['expiry'] <= $this->max_life_time)
                {
                    $this->session_expiry = $session['expiry'];//$session_data['expiry'];
                    $this->session_md5    = md5($session['data']);//md5($session_data['data']);
                    $GLOBALS['_SESSION']  = unserialize(stripslashes($session['data']));//unserialize($session_data['data']);
                    $GLOBALS['_SESSION']['user_id'] = $session['userid'];
                    $GLOBALS['_SESSION']['admin_id'] = $session['adminid'];
                    $GLOBALS['_SESSION']['user_name'] = $session['user_name'];
                    $GLOBALS['_SESSION']['user_rank'] = $session['user_rank'];
    ...
    function update_session()
    {
       ...
       /*  if (isset($data{255}))
        ...
        return $this->db->replace($this->session_id, array('expiry'=>$this->_time, 'ip'=>$this->_ip, 'userid'=>$userid, 'adminid'=>$adminid,'user_name'=>$user_name,'user_rank'=>$user_rank,'discount'=>$discount,'email'=>$email, 'data'=>$data), false, $this->max_life_time);
    }
...
/*     function delete_spec_admin_session($adminid)
    ...
    function delete_spec_admin_session($adminid)
    {
    	if (!empty($GLOBALS['_SESSION']['admin_id']) && $adminid)
    	{
    		...
    		$items = $all_items[$this->m_info]['items'];
   ...
   /*  function get_users_count()
    ...
    function get_users_count()
    {
    	...
    	return $count = $all_items[$this->m_info]['curr_items'];//由于有其他key的缓存，因此这只是个接近数值
    }

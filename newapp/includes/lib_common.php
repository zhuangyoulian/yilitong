...
{
    $sql = 'SELECT id as region_id, name as region_name FROM ' . $GLOBALS['ecs']->table('region') .
...
/***获取 cart 商品数量**/
function get_CartsNum(){
	if(empty($_SESSION['user_id'])){
		return 0;
		exit;
	}
	$sql= 'SELECT sum(c.goods_number) FROM ' . $GLOBALS['ecs']->table('cart') .
            " as c INNER JOIN " .$GLOBALS['ecs']->table('goods'). " as g ON g.goods_id = c.goods_id 
...
/**
 * [order_remark 记录订单备注&留言]
 * @Author   [zihang]
 * @DateTime [2016-03-01T16:33:20+0800]
 * @param    [type]                     $order_id    [订单id]
 * @param    [type]                     $type        [备注&留言]
 * @param    string                     $remark_note [信息]
 * @param    [type]                     $username    [操作人]
 * @return   [type]                                  [description]
 */
function order_remark($order_id,$type,$remark_note = '',$username = null){
    if(is_null($username)){
        $username = $_SESSION['admin_name'];
    }
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('order_remark') .
                ' (order_id, remark_user, type, remark_note, log_time) ' .
            "VALUES ('$order_id','$username','$type','$remark_note',".time().")";
    return $GLOBALS['db']->query($sql);
}

/**
 * [get_orderRemark 获取备注信息]
 * @Author   [zihang]
 * @DateTime [2016-03-02T13:47:19+0800]
 * @param    [type]                     $order_id [description]
 * @return   [type]                               [description]
 */
function get_orderRemark($order_id){
    if(is_null($order_id))return false;
    $sql = 'SELECT * FROM' .  $GLOBALS['ecs']->table('order_remark') .
    'WHERE order_id ='.$order_id;
    $res = $GLOBALS['db']->getAll($sql);
    foreach($res as $k=>$v){
        $res[$k]['log_time'] = date($GLOBALS['_CFG']['time_format'],$res[$k]['log_time']);
    }
    return $res;
}

// $json_string='{"\id":1,"\name":"jb51","\email":"admin@jb51.net","\interest":["wordpress","php"]} '; 
function postType($post){
    $obj = str_replace("\\","",$post);
    $obj = json_decode($obj);
    return $obj;
}
 ...
 * @return 分页
 */
function pagelist($pgaemode,$num, $curr_page, $array = array(), $perpage = 20, $setpages = 7, $class = 'a1') {
    ...
    if ($num > $perpage) {
        ...
        //$multipage .= ' <a class="{$class}">共 ' . $num . ' 条  ' . $curr_page . '/' . $pages . '页 每页 ' . $perpage . ' 条</a>';
		
        $multipage .= '<li> <a href="' . pageurl($pgaemode,0, $array) . '">首页 </a> </li>';
        if ($curr_page > 0) {
            $multipage .= '<li> <a href="' . pageurl($pgaemode,$curr_page - 1, $array) . '" aria-label="Previous"  class="{$class}">&lt;</a></li>';
            if ($curr_page == 1) {
                $multipage .= '<li class="active" ><a>1</a></li>';
            } elseif ($curr_page > 6 && $more) {
                $multipage .= '<li> <a href="' . pageurl($pgaemode,1, $array) . '">1</a>..</li>';
            } else {
                $multipage .= ' <li><a href="' . pageurl($pgaemode,1, $array) . '">1</a></li>';
            }
        }
        for ($i = $from; $i <= $to; $i++) {
            if ($i != $curr_page) {
                $multipage .= '<li> <a href="' . pageurl($pgaemode,$i, $array) . '">' . $i . '</a></li>';
            } else {
        ...
        if ($curr_page < $pages) {
            if ($curr_page < $pages - 5 && $more) {
                $multipage .= '<li> .. <a href="' . pageurl($pgaemode,$pages, $array) . '">' . $pages . '</a></li> <li><a href="' . pageurl($pgaemode,$curr_page + 1, $array) . '" aria-label="Next" class="{$class}">&gt;</a></li>';
            } else {
                $multipage .= '<li> <a href="' . pageurl($pgaemode,$pages, $array) . '">' . $pages . '</a></li> <li><a href="' . pageurl($pgaemode,$curr_page + 1, $array) . '" aria-label="Next" class="{$class}">&gt;</a></li>';
            }
        } elseif ($curr_page == $pages) {
            $multipage .= '<li class="active"> <a>' . $pages . '</a></li> <li><a href="' . pageurl($pgaemode,$curr_page, $array) . '" aria-label="Next" class="{$class}">&gt;</a></li>';
        } else {
            $multipage .= '<li> <a href="' . pageurl($pgaemode,$pages, $array) . '">' . $pages . '</a></li> <li><a href="' . pageurl($pgaemode,$curr_page + 1, $array) . '" aria-label="Next" class="{$class}">&gt;</a></li>';
        }
        $multipage .= '<li> <a href="' . pageurl($pgaemode,$pages, $array) . '">尾页 </a> </li>';
    }
 ...
 * @return 完整的URL路径
 */
function pageurl($pgaemode,$page, $array = array()) {
    $url =$pgaemode.'?' ;
    if (is_array($array)) {
        $array['page'] = $page;
        if (!strexists($url,"?")) {
...
*
*/
function get_goodscomment_list($goods_id,$files='',$limit='',$order=''){
	$where=" id_value =".$goods_id. " and `status` = 1";
	$order=empty($order) ? ' comment_id ' : $order;
...
function httppost_json($url, $data_string) {
...
function post_order($url,$data){
	
	$string = httppost($url,$data);
	$xml = (array)simplexml_load_string($string);
	logpush($url,$data,var_export($xml, TRUE));
	set_push_order_status($data,$xml);
	return $xml[0];
}

function post_tracking_no($url,$order_no){
	$json = "strJson=" .'{"request": {"api_type": "LogisticsSearch","api_key": "'.wms_key().'","order_id": "'.$order_no.'"}}';	
	//return httppost_json($url, $json);
	$string = httppost($url,$json);
	$xml = (array)simplexml_load_string($string);
	$arr = json_decode($xml[0]) ;
	$content = $arr->response;
...
function logpush($url,$coent,$rs){
	$file = "/temp/pushwms.txt";
...
function set_push_order_status($data,$rs){
	
	preg_match('/(.*?)\"order_id\":\"(.*?)\",(.*?)/',$data,$res);
	$orderno = $res[2];
	
	$sql = "update ecs_order_info set repush_times=repush_times+1 where order_sn='$orderno' and push_status=0";
	if($rs[0]=='T'){
		$sql = "update ecs_order_info set push_status=1 where order_sn='$orderno' and push_status=0";
	}
	if($rs[0]=='订单已存在！'){
		$sql = "update ecs_order_info set push_status=1 where order_sn='$orderno' and push_status=0";
	}
...
function post_order_basic_info($url,$data){

	$string = httppost($url,$data);
	$xml = (array)simplexml_load_string($string);
	
	$arr = (array)json_decode($xml[0]);
	
	$file = "/temp/modify_order.txt";
	...
	return (array)$arr['response'];
}

function post_cancel_order($url,$data){
...
function curl_get($url, $get = NULL, $options = array()) 
{    
    $defaults = array( 

...
class cls_mysql
{
    ...
    function query($sql, $type = '')
    {
       ...
       /* if(true)
        {
            $logfilename = $this->root_path . 'temp' . '/mysql_query_' . $this->dbhash . '_' . date('Y_m_d') . '.log';
            $str = $sql . "\n\n";
 ...
 */
    	$others = "";
    	if(empty($message)){
    		$others = var_export($this->error_message,true);
    	}

    		$logfilename = $this->root_path . 'temp' . '/mysql_query_error' . $this->dbhash . '_' . date('Y_m_d') . '.log';
...
function logquery($sql){
	$y=date('Y', time());
	$m=date('m', time());
	$file=ROOT_PATH."temp/querylog";
	!is_dir($dir) && @mkdir(iconv("UTF-8", "GBK", $file),0777,true);
	$file=$file."/{$y}_{$m}_querylog.txt";
	$fp = fopen($file,"a");
	flock($fp, LOCK_EX) ;

...
if($action == 'login_log'){
	if(!empty($userinfo['userid'])){
		...
		$sql = "INSERT INTO " .$GLOBALS['ecs']->table('login_log'). '(`user_id`,`user_name`, `user_phone`,`log_ip`,`log_time`,`source`)
		VALUES('.$userInfo[user_id].', "'.$userInfo[nickname].'","'.$userInfo[mobile].'", "'.$log_ip.'","'.$add_time.'","'.$source.'")';
...
elseif ($action=='check_login_status'){
	if(!empty($userinfo['userid'])){
		...
		$sql =" select user_id ".$GLOBALS['ecs']->table('users')." where user_id ={$user_id} and $device_token={$device_token} ";
        $user_id= $db->getOne($sql);
        if(!empty($user_id)){
            $sql="UPDATE ".$GLOBALS['ecs']->table('users')." SET last_login= ".time() ." where user_id = {$user_id} ";
            $rs3=$GLOBALS['db']->query($sql);
            exit($json->json_encode_ex(['result'=>0,'info' => 'OK']));
        }else{
            exit($json->json_encode_ex(['result'=>0,'info' => '重新登录']));
        }
	}else{
		$results = array(
				'result'=>1,
...
elseif($action=='login'){
	...
	//	用户名密码不为空
	if (!empty($phone) && !empty($password)){
		$sql = "SELECT user_id,password,nickname,head_pic,device_token,app_type,recommend_code,FManagerId FROM " .$GLOBALS['ecs']->table('users')." where mobile={$phone}";
   ...
   if($app_type=='1'){
	...
	if(!empty($device_token) && !empty($app_type)){
...
elseif($action=="user_binding"){
	...
	if(!empty($users['mobile'])){
		//登录成功修改状态
		if(!empty($app_type) && !empty($device_token) && $device_token!=$users['device_token']){
...
elseif($action=="get_user_info"){
	$userid=$userinfo['userid'];
	$sql="select user_id,sex,birthday,mobile,nickname,head_pic,recommend_code,FManagerId,parent_id from ".$GLOBALS['ecs']->table('users')." where user_id={$userid} ";
...
elseif($action=="new_register"){
	...
	$sql = "SELECT business_level,user_id,mobile,password,nickname,head_pic,device_token,app_type,openid,oauth FROM " .$GLOBALS['ecs']->table('users')." $where ";

	$user_data = $GLOBALS['db']->getRow($sql);
		
	$users['mobile']=!empty($phone) ? $phone : "";
...
//	$users['head_pic']=!empty($head_pic) ? $head_pic : $users['head_pic'];
	...
	if(!empty($user_data)){
		//获得推荐码
		$recommend_code = 'us'.$user_data['user_id'];
		$GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('users')." SET recommend_code= '{$recommend_code}' where user_id = {$user_data['user_id']} ");
		
		$users['recommend_code'] = $recommend_code;

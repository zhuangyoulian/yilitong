...
elseif($action=='conf_order'){
	...
	if($order){
		...
		$time = time();
	$sql = "UPDATE " . $GLOBALS['ecs']->table('order') . " set order_status = 2 , confirm_time = {$time} WHERE user_id={$userid} AND order_id=".$order['order_id'];
		$flag=$GLOBALS['db']->query($sql);
	}
	if($flag){
...
elseif($action=='back_money'){
	...
	$where=" pay_status=1 ";
	$sql="SELECT parent_id,order_amount,order_id,order_sn,order_status,supplier_id from ".$GLOBALS['ecs']->table('order')." where order_id={$order_id} AND user_id={$userid} and ($where)";
	...
	if(!empty($goods['spec_key'])){
		$sql="select count(*) from ".$GLOBALS['ecs']->table('back_order')." where order_id={$order_id} AND order_sn={$order[order_sn]} AND user_id={$userid} AND spec_key='{$goods[spec_key]}'";
	}else{
		$sql="select count(*) from ".$GLOBALS['ecs']->table('back_order')." where order_id={$order_id} AND order_sn={$order[order_sn]} AND user_id={$userid}";
	}
	$count=$db->getOne($sql);
	if($count){
...
elseif($action=='back_goods'){
	...
	$spec_key=$_REQUEST['spec_key'];
	 ...
	 if(empty($note) || empty($number) || empty($type) || empty($order_id) || empty($goods_id)){
...
elseif($action=="back_order_detail"){
	...
	$file="B.back_id,B.rec_id,A.order_id,A.order_sn,A.type,A.reason,A.addtime,A.status,A.remark,A.imgs,B.goods_name,B.send_number,B.money,A.refund_shipping_no,A.refund_shipping_no";
	if(!empty($spec_key)){
		$sql="SELECT {$file} from ".$GLOBALS['ecs']->table('back_order')." as A left join ".$GLOBALS['ecs']->table('back_goods')." as B on B.back_id=A.id where A.order_id={$order_id} AND A.user_id={$userid} AND A.goods_id={$goods_id} AND A.spec_key='{$spec_key}'";
	}else{
		$sql="SELECT {$file} from ".$GLOBALS['ecs']->table('back_order')." as A left join ".$GLOBALS['ecs']->table('back_goods')." as B on B.back_id=A.id where A.order_id={$order_id} AND A.user_id={$userid} AND A.goods_id={$goods_id} ";
	}
	...
	if(!empty($row['rec_id'])){
		$rec_id=$row['rec_id'];
		$sql="SELECT count(*) from ".$GLOBALS['ecs']->table('back_good_logistics')." where rec_id={$rec_id} AND user_id={$userid}";
		$num=$db->getOne($sql);
		if($num>0){
			$row['is_logistics']="1";
		}else{
		...
		$sql="SELECT rec_id,add_time,user_id,supplier_id,content from ".$GLOBALS['ecs']->table('back_msg')." where rec_id={$rec_id}";
		$message=$db->getAll($sql);
	}
	
	if(!empty($row)){
		$row['replay_time']=$row['addtime'];
		$rs=array('result'=>'1','info'=>'请求成功','data'=>$row,'message'=>$message);
		exit($json->json_encode_ex($rs));
	}else{
...
function get_order_sum($userid,$type){
	...
	{
		case 0:
			$where.=" O.order_status=0 AND O.pay_status = 0 AND O.pay_code !='cod' ";
			break;
		case 1:
...
function insert_comments($data){
	$sql="SELECT count(*) from ".$GLOBALS['ecs']->table('order')." as A left join ".$GLOBALS['ecs']->table('order_goods')." as B on A.order_id=B.order_id where A.user_id={$data[user_id]} AND A.order_id={$data['order_id']} AND order_status=2";
	$count=$GLOBALS['db']->getOne($sql);
	if($count<1){
		return false;
	} 
	$sql="SELECT count(*) from ".$GLOBALS['ecs']->table('comment')." where order_id={$data['order_id']}  AND user_id={$data[user_id]} AND goods_id={$data['goods_id']}";
	...
	$sql="SELECT spec_key_name from ".$GLOBALS['ecs']->table('order_goods')." where order_id={$data['order_id']} ";
	$arr['spec_key_name']=$GLOBALS['db']->getOne($sql);

    $sql = "UPDATE " . $GLOBALS['ecs']->table('order_goods') . " set is_comment = 1 WHERE goods_id={$data[goods_id]} AND order_id={$data[order_id]}";
    $GLOBALS['db']->query($sql);

...
elseif($action=='check_cart'){
	...
	$rs=array('result'=>'1','info'=>'请求成功','allprice'=>$need_pay,'subtotal'=>$price,'all_number'=>$all_number,'cart_ids'=>$ids,'good_list'=>$rs,'address'=>$address);
	exit($json->json_encode_ex($rs));
}
       ...
       if($sid){//判断是不是平台直营
		...
		if(!empty($rs)){
			foreach ($rs as $key =>$val){
				...
				$sql =" SELECT '$new_son_id', goods_id, goods_name,prom_type,prom_id, goods_sn, goods_num, market_price, ".
...
elseif($action=="buy_now"){
	...
	$rs=array('result'=>'1','info'=>'请求成功','goods'=>$goods,'address'=>$address,'allprice'=>$need_pay,'subtotal'=>$price,'all_number'=>$number);
	exit($json->json_encode_ex($rs));
}
//立即购买生成订单接口
elseif ($action=='buy_now_order'){
	...
	$sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
			...
			"goods_price,spec_key,spec_key_name,prom_type,prom_id,goods_amount) values('{$new_order_id}', {$goods_id}, '{$goods[goods_name]}', '{$goods[goods_sn]}', {$number}, {$goods[market_price]},{$data[price]},'{$row[key]}','{$row[key_name]}','$act_rs[prom_type]','$act_rs[prom_id]','$order_amount')";
...
function get_condition_cartlist($userid,$ids=''){
	...
	$file="C.id,C.goods_id,C.market_price,C.goods_price,C.spec_key,C.prom_type,C.prom_id,C.goods_num,C.goods_sn,C.goods_name,C.add_time,C.spec_key_name,";
	$file.="G.store_count,G.original_img,G.supplier_id,G.is_free_shipping,G.examine";
	$sql="select {$file} from ".$GLOBALS['ecs']->table('cart')." as C left join ".$GLOBALS['ecs']->table('goods')." as G ON C.goods_id=G.goods_id where id in ($ids) AND C.user_id={$userid} GROUP BY G.goods_id,G.supplier_id,C.spec_key,C.prom_type,C.prom_id ";
...
function get_cart_list($userid){
	...
	$file.="G.store_count,G.prom_type,G.prom_id,G.original_img,G.supplier_id,G.is_designer,G.is_free_shipping";

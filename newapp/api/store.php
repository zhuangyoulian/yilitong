...
if($action=='index'){
	...
	$sql = "SELECT value FROM ".$GLOBALS['ecs']->table('supplier_config')." where supplier_id ='".$supplier_id."' and name = 'store_desc'";
	$supplier_config = $db->getOne($sql);
	...
	$store['phone_number']=!empty($store['phone_number']) ? $store['phone_number'] :'';
	$store['logo']=!empty($store['logo']) ? $store['logo'] :'';
	$store['introduction']=!empty($supplier_config) ? $supplier_config : $store['business_sphere'];

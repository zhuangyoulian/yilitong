...
class AgenGoods extends Model {
    ...
    {    
         ...
         {                          
             ...
             $goodsImagesArr = Db::name('AgenGoodsImages')->where("goods_id = $goods_id")->column('img_id,image_url'); // 查出所有已经存在的图片
             
             // 删除图片
             foreach($goodsImagesArr as $key => $val)
             {
                 if(!in_array($val, $goods_images))
                     Db::name('AgenGoodsImages')->where("img_id = {$key}")->delete(); //
             }
             // 添加图片
             foreach($goods_images as $key => $val)
             {
                 ...
                 {                 
                        ...
                        Db::name("AgenGoodsImages")->insert($data); // 实例化User对象
                 }
             }
         }
         // 查看主图是否已经存在相册中
         $original_img = I('original_img');
         $c = Db::name('AgenGoodsImages')->where("goods_id = $goods_id and image_url = '{$original_img}'")->count();
         if($c == 0 && $original_img)
         {
             Db::name("AgenGoodsImages")->add(array('goods_id'=>$goods_id,'image_url'=>$original_img));
         }
      //   delFile("./public/upload/goods/thumb/$goods_id"); // 删除缩略图
         
         // 商品规格价钱处理        
         Db::name("AgenGoodsPrice")->where('goods_id = '.$goods_id)->delete(); // 删除原有的价格规格对象
         if(I('item/a'))
         {
             $spec = Db::name('AgenSpec')->column('id,name'); // 规格表
             $specItem = Db::name('AgenSpecItem')->column('id,item');//规格项
             ...
             foreach(I('item/a') as $k => $v)
             {
                   ...
                   $dataList[] = ['goods_id'=>$goods_id,'key'=>$k,'key_name'=>$v['key_name'],'price'=>$v['price'],'store_count'=>$v['store_count'],'sku'=>$v['sku'],'quantity'=>$v['quantity']];
                    // // 修改商品后购物车的商品价格也修改一下
                    // Db::name('agen_cart')->where("goods_id = $goods_id and spec_key = '$k'")->save(array(
                    //         'market_price'=>$v['price'], //市场价
                    //         'goods_price'=>$v['price'], // 本店价
                    //         'member_goods_price'=>$v['price'], // 会员折扣价                        
                    //         ));                   
             }            
             Db::name("AgenGoodsPrice")->insertAll($dataList);
         ...
         if(I('item_img/a'))
         {    
             Db::name('AgenSpecImage')->where("goods_id = $goods_id")->delete(); // 把原来是删除再重新插入
             foreach (I('item_img/a') as $key => $val)
             {                 
                 Db::name('AgenSpecImage')->insert(array('goods_id'=>$goods_id ,'spec_image_id'=>$key,'src'=>$val));
             }                                                    
         }

...
class AgenSpec extends Model {
   ...
   /**
    ...
    public function afterSave($id)
    {
        ...
        $model = Db::name("AgenSpecItem"); // 实例化User对象
        ...
        foreach($db_items as $key => $val)
        {
            ...
            {       
                ...
                Db::name("AgenGoodsPrice")->where("`key` REGEXP '^{$key}_' OR `key` REGEXP '_{$key}_' OR `key` REGEXP '_{$key}$' or `key` = '{$key}'")->delete(); // 删除规格项价格表
                Db::name("AgenSpecItem")->where('id='.$key)->delete(); // 删除规格项
            }
        }        
    }    
}

...
class Works extends Base {
    ...
    */
    public function index(){
		...
		$this->assign('GoodsCategory',$GoodsCategory); //商品分类
		$this->assign('WCategory',$WCategory);	//作品分类
    ...
    */
    public function WorksDetails(){
        ...
        Db::name('works')->where("works_id", $works_id)->setInc('click_count'); //统计点击数
    ...
    public function WorksComment(){
        ...
        if(empty($data['content'])){
            $rs=array('status'=>'-1','msg'=>'评论失败,请重试!!!');
            exit(json_encode($rs));
        }

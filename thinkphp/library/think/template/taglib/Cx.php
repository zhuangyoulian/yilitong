...
class Cx extends Taglib
{
    ...
    protected $tags = [
        ...
        'ylt'     => ['attr'=>'sql,key,item,result_name','close'=>1,'level'=>3], // sql 万能标签
        'adv'        => ['attr'=>'limit,order,where,item','close'=>1],
    ];
    
	
	public function tagYlt($tag, $content)
        {
            ...
            $t  =  !empty($tag['t']) ? $tag['t'] : YLT_CACHE_TIME;// 缓存时间
	...
	public function tagAdv($tag, $content)
        ...
        $ad_id = !empty($tag['ad_id']) ? $tag['ad_id'] : '0';
        ...
        $str = '<?php ';
        $str .='$where ="'.$where.'";';
        $str .= '$ad_position = M("ad_position")->cache(true,YLT_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");';
        $str .= '$result = M("ad")->where("$where and enabled = 1  ")->order("orderby desc")->cache(true,YLT_CACHE_TIME)->limit("'.$limit.'")->select();';
        $str .= '

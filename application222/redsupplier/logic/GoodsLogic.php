...
namespace ylt\redsupplier\logic;
...
class GoodsLogic extends Model
{
     ...
     * @return  mix
     */
    public function goods_cat_list($supplier_id,$cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
                global $goods_category, $goods_category2;
                $sql = "SELECT * FROM  __PREFIX__supplier_goods_category where supplier_id = $supplier_id ORDER BY parent_id , sort_order ASC";
    ...
    {
        $GoodsCategory = Db::name("SupplierGoodsCategory"); // 实例化User对象
        $cat = $GoodsCategory->where("id = $id")->find(); // 找出他自己
        // 刚新增的分类先把它的值重置一下
        if($cat['parent_id_path'] == '')
        {
            ($cat['parent_id'] == 0) && Db::execute("UPDATE __PREFIX__supplier_goods_category set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类
            Db::execute("UPDATE __PREFIX__supplier_goods_category AS a ,__PREFIX__goods_category AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = $id");
            $cat = $GoodsCategory->where("id = $id")->find(); // 从新找出他自己
        }

        if($cat['parent_id'] == 0) //有可能是顶级分类 他没有上级
        {
            $parent_cat['parent_id_path'] =   '0';
            $parent_cat['level'] = 0;
        }
        else{
            $parent_cat = $GoodsCategory->where("id = {$cat['parent_id']}")->find(); // 找出他上级的parent_id_path
        }
        $replace_level = $cat['level'] - ($parent_cat['level'] + 1); // 看看他 相比原来的等级 升级了多少  ($parent_cat['level'] + 1) 他上级等级加一 就是他现在要改的等级
        $replace_str = $parent_cat['parent_id_path'].'_'.$id;
        Db::execute("UPDATE `__PREFIX__supplier_goods_category` SET parent_id_path = REPLACE(parent_id_path,'{$cat['parent_id_path']}','$replace_str'), level = (level - $replace_level) WHERE  parent_id_path LIKE '{$cat['parent_id_path']}%'");
    }
               ...
               foreach ($v as $k2 => $v2)
               {
                   ...
                   if(array_key_exists($tmp_key , $old_goods_attr)) // 如果这个属性 原来就存在
                   {
                       if($old_goods_attr[$tmp_key]['attr_price'] != $attr_price) // 并且价格不一样 就做更新处理
                       {
                            ...
                            $GoodsAttr->where("goods_attr_id = $goods_attr_id")->update(array('attr_price'=>$attr_price));
                       }
                   }
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
       ...
       foreach ($clo_name as $k => $v)
       {
        ...
        $str .="<td><b>价格</b><input type='text' class='setSamePrice' id='setSamePrice' value='0' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")'></td>
               ...
               <td><b>起售量</b><input type='text' class='setSameQuantity' id='setSameQuantity' value='0' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")'></td>
             </tr>";
       // 显示第二行开始
          foreach ($spec_arr2 as $k => $v)
       {
			...
			$keySpecGoodsPrice[$item_key][quantity] ? false : $keySpecGoodsPrice[$item_key][quantity] = 1; //起售量默认为1
            ...
            $str .="<td><input name='item[$item_key][store_count]' class='goodsSpecNum' value='{$keySpecGoodsPrice[$item_key][store_count]}' maxlength='7' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")'/></td>";
            $str .="<td><input name='item[$item_key][quantity]' class='setSameQuantity' value='{$keySpecGoodsPrice[$item_key][quantity]}' maxlength='7' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")'/></td>";
            $str .="
    ...
    function GetSpecCheckboxList($type_id, $checked = array()){
        ...
        //$list = Db::name('Spec')->where("1=1")->order('`order` desc')->select();
        $str = '';

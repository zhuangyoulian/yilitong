...
class OrderLogic extends Model
{
     ...
     */
    public function getOrderList($condition,$order='',$start=0,$page_size=20,$where){
        $res = Db::name('red_order')->where($condition)->where($where)->limit("$start,$page_size")->order($order)->select();
        foreach ($res as $key => $value) {
            //判断红礼组合礼包中是否有该商家的商品
            if ($value['is_group'] == 1) {
                $goods_id = Db::name('red_order_goods')->where('order_id',$value['order_id'])->field('goods_id')->select();
                foreach ($goods_id as $ke => $valu) {
                    $goods_group = Db::name('red_goods')->where('goods_id',$valu['goods_id'])->value('goods_group');
                    if ($goods_group) {
                        $goods_group_id[]=$goods_group;
                    }
                }
                if ($goods_group_id) {
                    $red_supplier_id= Db::name('red_goods')->where("goods_id in ($goods_group_id[0])")->field('red_supplier_id,goods_id')->select();
                    foreach ($red_supplier_id as $k => $val) {
    ...
    public function getOrderInfo($order_id)
    {
        ...
        $order['max_goods_num'] = Db::name('red_order_goods')->where("order_id = $order_id")->max('goods_num');
    ...
    public function get_spec_goods($goods_id_arr){
    	if(!is_array($goods_id_arr)) return false;
    		...
    		{
    			$arr = array();
    			$goods = Db::name('red_goods')->where("goods_id = $key")->find();
    ...
    public function orderActionLog($order_id,$action,$note=''){    	
        ...
        return Db::name('red_order_action')->add($data);//订单操作记录
    }
    ...
    public function orderProcessHandle($order_id,$act,$ext=array()){
    	$updata = array();
        $order_sn = Db::name('red_order')->where("order_id = $order_id")->field("order_sn,source")->find();
    	switch ($act){
    		case 'pay': //付款
                update_pay_status($order_sn['order_sn'],$ext); // 调用确认收货按钮
    			return true;    			
        ...
        if ($order_sn['source'] == '一礼通') {
            Db::name('order')->where("order_id=$order_id")->update($updata);
        }
    	return Db::name('red_order')->where("order_id=$order_id")->update($updata);//改变订单状态
    }
    ...
    */
    ...
    function order_pay_cancel($order_id)
    {
    	...
    	$order = Db::name('red_order')->where("order_id = $order_id")->find();
    	// 增加对应商品的库存
        $orderGoodsArr = Db::name('RedOrderGoods')->where("order_id = $order_id")->select();
    	foreach($orderGoodsArr as $key => $val)
    	{
    		...
    		{   // 先到规格表里面增加数量 再重新刷新一个 这件商品的总数量
    			...
    			Db::name('red_Goods')->where("goods_id = {$val['goods_id']}")->setInc('store_count',$val['goods_num']); // 增加商品总数量
    		}
    		Db::name('red_Goods')->where("goods_id = {$val['goods_id']}")->setDec('sales_sum',$val['goods_num']); // 减少商品销售量
    		//更新活动商品购买量
    	...
    	Db::name('red_order')->where("order_id=$order_id")->update(array('pay_status'=>0));
    ...
    public function deliveryHandle($data){
		...
		foreach ($orderGoods as $k=>$v){
			...
			if($v['is_send'] == 0 && in_array($v['rec_id'],$selectgoods)){
				$res['is_send'] = 1;
				$res['delivery_id'] = $did;
                $r = Db::name('red_order_goods')->where("rec_id=".$v['rec_id'])->update($res);//改变订单商品发货状态
                if($order['source'] == '一礼通'){
                    $res_s['is_send'] = 1;
                    $res_s['delivery_id'] = $didd;
                    $order_goods_rid = DB::name('red_order_goods')->where("rec_id=".$v['rec_id'])->value('order_goods_rid');
                    $array = Db::name('order_goods')->where("rec_id=".$order_goods_rid)->update($res_s);//改变订单商品发货状态
                    if (!$array) {
                        $this->error('订单商品不存在！');
                    }
                }

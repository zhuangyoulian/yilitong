<?php
namespace ylt\redsupplier\model;
use think\Model;
use think\Db;
use think\Request;
class Goods extends Model {
    ...
    {            
         ...
         {                          
             ...
             $goodsImagesArr = Db::name('GoodsImages')->where("goods_id = $goods_id")->getField('img_id,image_url'); // 查出所有已经存在的图片
       ...
       //  delFile("./public/upload/goods/thumb/$goods_id"); // 删除缩略图
         ...
         if(I('item/a'))
         {
             $spec = Db::name('Spec')->getField('id,name'); // 规格表
             $specItem = Db::name('SpecItem')->getField('id,item');//规格项

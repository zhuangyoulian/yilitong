...
class RedSpec extends Model {
   ...
   /**
    ...
    public function afterSave($id)
    {
        ...
        $model = Db::name("RedSpecItem"); // 实例化User对象
        ...
        foreach($db_items as $key => $val)
        {
            ...
            {       
                ...
                Db::name("RedGoodsPrice")->where("`key` REGEXP '^{$key}_' OR `key` REGEXP '_{$key}_' OR `key` REGEXP '_{$key}$' or `key` = '{$key}'")->delete(); // 删除规格项价格表
                Db::name("RedSpecItem")->where('id='.$key)->delete(); // 删除规格项
            }
        }        
    }    
}

...
//        send_http_status('310');
        ...
        $path = UPLOAD_PATH."sqldata/ylt_tables_" . date("Ymd").get_rand_str(3,0);
        ...
        if (!file_put_contents($file, $file_str, FILE_APPEND)) {
            $this->error("备份文件写入失败！", Url::build('Tools/index'));
        }
        ...
        foreach ($tables as $table) {
            ...
            for ($i = 1; $i <= $page; $i++) {
                ...
                foreach ($query as $val) {     
                    ...
                    $path = UPLOAD_PATH."sqldata/ylt_{$table}_" . date("Ymd").get_rand_str(3,0);
    ...
    private function getRestoreFiles() {
    	$sqlfilepre = I('sqlfilepre','');//获取sql文件前缀 ylt_tp_article_cat_20170206pno_4.sql.html
    	if (empty($sqlfilepre)) {
    ...
    public function restoreData() {
    		...
    		while (!feof($file)) {
	    	   ...
	    	   $execute++;
	    	   		if ($execute > 500) {
			    		...
			    		//echo json_encode(array("status" => 1, "info" => '如果导入SQL文件卷较大(多)导入时间可能需要几分钟甚至更久，请耐心等待导入完成，导入期间请勿刷新本页，当前导入进度：<font color="red">已经导入' . $imported . '条Sql</font>', "url" => Url::build('Database/restoreData', array(get_randomstr(5) => get_randomstr(5)))));
			    		$this->success('如果SQL文件卷较大(多),则可能需要几分钟甚至更久,<br/>请耐心等待完成，<font color="red">请勿刷新本页</font>，<br/>当前导入进度：<font color="red">已经导入' . $imported . '条Sql</font>', Url::build('Tools/restoreData', array(get_rand_str(5,0) => get_rand_str(5,0))));
			    		exit();
					}
	    		} else {
	    			$sql .= $tem;
    	...
    	unset($_SESSION['cacheRestore']);
    	$this->success("导入成功，耗时：{$time} 秒钟", Url::build('Tools/restore'));
    }
    ...
    public function optimize() {
    	...
    	$this->success("优化表成功" . $strTable, Url::build('Tools/index'));
    ...
    public function repair() {
    	...
    	$this->success("修复表成功" . $strTable, Url::build('Tools/index'));
	...
	public function restoreUpload()
	{
		...
		if($info){
			if (file_exists($save_url.'/'.$info->getFilename)) {
				$this->success("上传成功", Url::build('Tools/restore'));
			} else {
				$this->error('文件不存在');
			}
		}else{
    ...
    public function delSqlFiles() {
    	...
    	if($a){
    		$this->success("已删除：" . implode(",", $files), Url::build('Tools/restore'));
    	}else{
    		$this->error("删除失败：" . implode(",", $files), Url::build('Tools/restore'));
    	}	
    }
    ...
    public function regionHandle(){
    	$data = I('post.');
    	$id = I('id');
    	$referurl =  isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : Url::build("Tools/region");
    	if(empty($id)){
    		$data['level'] = $data['level']+1;
    		if(empty($data['name'])){
    			$this->error("请填写地区名称", $referurl);
    		}else{
    			$res = Db::name('region')->where("parent_id = ".$data['parent_id']." and name='".$data['name']."'")->find();
    			if(empty($res)){
    				Db::name('region')->add($data);
    		...
    		Db::name('region')->where("id=$id or parent_id=$id")->delete();
    		$this->success("操作成功", $referurl);
    	}
    }
}
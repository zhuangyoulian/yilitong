...
class OrderLogic extends Model
{
    ...
    public function getOrderList($condition,$order='',$start=0,$page_size=20){
        if($condition['prom_id'] > 0)
            $res = Db::name('order')->alias('o')->join('order_goods g','o.order_id = g.order_id','left')->where($condition)->limit("$start,$page_size")->order('o.order_id')->select();
        else
	 ...
	 */
	 private function ShippingName($code){
         ...
         // $url='http://www.kuaidi100.com/autonumber/auto?num=7818882365&key=kvDyZIPg551';
     ...
     */
    public function refund_for_weixin($out_refund_no,$out_trade_no,$refund_fee,$total_fee)
    {
     ...
     * @param $user_id 用户ID
     */
    public function refund_for_alipay($order_sn,$money,$out_request_no = 1,$user_id){


        $url='http://www.yilitong.com/newapp/api/pay.php?act=return_alipay';

        $userinfo = Db::name('users')->where(array('user_id'=>$user_id))->find();

        $pwd = encrypt($userinfo['mobile']);

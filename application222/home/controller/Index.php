...
class Index extends Base {
    ...
    public function index(){
        ...
        if(strpos($_SERVER[REQUEST_URI],'dist') == false){ 
            header("Location: ".Url::build('/dist/index'));
            exit;
        }
    }
     ...
     */
  	public function dist(){
        // banner
        $fields = 'ad_code,ad_link,ad_name';
        $ad_banner = Db::name('ad')->where('pid = 43 and enabled =1')->field($fields)->order('orderby desc')->select();
        ...
        $rs=array('result'=>'1','info'=>'请求成功','res'=>$res,'is_hot'=>$is_hot,'brand_list'=>$brand_list,'supplier_list'=>$supplier_list,'goods_category_tree'=>$this->goods_category_tree,'scenario_category_tree'=>$this->scenario_category_tree,'scheme'=>$scheme,'article'=>$article,'ad_banner'=>$ad_banner);
        exit(json_encode($rs));
    ...
    public function config(){
        $rs=array('result'=>'1','info'=>'请求成功','config'=>$this->config);
        exit(json_encode($rs));
    }

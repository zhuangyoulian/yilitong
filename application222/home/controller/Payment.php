...
class Payment extends Base
{
    ...
    public function  __construct() {
        ...
        if(empty($this->pay_code)){
            exit(json_encode(array('status'=>'-2','info'=>'参数错误')));
            // $this->error("参数错误");
        }
    ...
    public function getCode(){
        ...
        if($order['pay_status'] == 1){
            exit(json_encode(array('status'=>'-1','info'=>'此订单，已完成支付')));
        ...
        $rs=array('status'=>'1','info'=>'请求成功','code_str'=>$code_str,'order_id'=>$order_id);
        exit(json_encode($rs));

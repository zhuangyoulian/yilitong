...
class RedGift extends Base {
    ...
    */
    ...
    public function index(){
        ...
        //当季热推 按排序查询4条数据
        $is_hot_o = Db::name('red_goods')->field($field)->where('examine = 1 and is_recommend =1 and is_on_sale = 1 and is_designer = 0 and sort = 1 ')->find();
        $is_hot_t = Db::name('red_goods')->field($field)->where('examine = 1 and is_recommend =1 and is_on_sale = 1 and is_designer = 0 and sort = 2 ')->find();
        $is_hot_s = Db::name('red_goods')->field($field)->where('examine = 1 and is_recommend =1 and is_on_sale = 1 and is_designer = 0 and sort = 3 ')->find();
        $is_hot_f = Db::name('red_goods')->field($field)->where('examine = 1 and is_recommend =1 and is_on_sale = 1 and is_designer = 0 and sort = 4 ')->find();
    ...
    public function goodsList(){ 
        ...
        $filter_goods_id = Db::name('red_goods')->where(['is_on_sale'=>1,'examine'=>1,'is_designer'=>0,'cat_id'=>['in',implode(',', $cat_id_arr)]])->cache(true)->column("goods_id");
        ...
        if (!$id) {
            $filter_goods_id = Db::name('red_goods')->where(['is_on_sale'=>1,'examine'=>1,'is_designer'=>0])->order("$sort $sort_asc")->field($field)->column("goods_id");
            $count =count($filter_goods_id);
            $goods_list = Db::name('red_goods')->where("goods_id","in", implode(',', $filter_goods_id))->order("$sort $sort_asc")->field($field)->limit($firstRow.','.$count_html)->select();
            //获取一级分类，判断是否为家居分类
            foreach ($goods_list as $key => $value) {
        ...
        $rs=array('result'=>'1','info'=>'请求成功','get_category'=>$get_category,'goods_list'=>$goods_lists,'cat_id'=>$id,'count'=>$count);
        exit(json_encode($rs));
    }
    ...
    public function search(){
        ...
        if ($keywords) {
            ...
            $goods = Db::name('red_goods')->where($where)->field("goods_id,goods_name,goods_thumb,shop_price,market_price")->select();
        }
        $rs=array('result'=>'1','info'=>'请求成功','goods'=>$goods);
        exit(json_encode($rs));
    }
    ...
    public function goodsInfo(){
        ...
        $recommend_goods =  Db::name('red_goods')->field('goods_id,goods_name,goods_thumb,shop_price,market_price,cat_id')->where('is_recommend = 1 and examine =1 and is_on_sale = 1')->order('goods_id desc')->limit(5)->select(); // 推荐
    ...
    {   
        ...
        foreach ($cartList as $k => $val) {
            ...
            foreach ($val['list'] as $key => $va) {
                $goods = Db::name('RedGoods')->where(array('goods_id' => $va['goods_id'], 'is_on_sale' => '1'))->find();
                if ($goods) {

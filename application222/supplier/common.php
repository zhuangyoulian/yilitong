...
function getAdminInfo($admin_id){
	return Db::name('supplier_user')->where("admin_id", $admin_id)->find();
}
...
{            
    $navigate = include APP_PATH.'supplier/conf/navigate.php';
    $location = strtolower('supplier/'.CONTROLLER_NAME);
    $arr = array(
...
function getAllMenu(){
	return	array(
			...
			'goods' => array('name' => '商品管理', 'icon'=>'fa-book', 'sub_menu' => array(
					array('name' => '店铺分类', 'act'=>'categoryList', 'control'=>'Goods'),
            ...
            'purchase' => array('name' => '询报价管理', 'icon'=>'fa-flag', 'sub_menu' => array(
                  array('name' => '我的询价信息', 'act'=>'purchase_list', 'control'=>'Purchase'),
                array('name' => '我的报价信息', 'act'=>'supply_list', 'control'=>'Purchase'),
            )),
			'system' => array('name'=>'商店设置','icon'=>'fa-cog','sub_menu'=>array(
					array('name'=>'商店设置','act'=>'index','control'=>'System'),
					array('name'=>'入驻信息','act'=>'supplier_info','control'=>'System'),
					
			)),
			// 'promotion' => array('name'=>'促销设置','icon'=>'fa-money','sub_menu'=>array(
			// 		// array('name'=>'满减设置','act'=>'prom_goods_list','control'=>'Promotion'),
			// 		array('name'=>'优惠券设置','act'=>'index','control'=>'Coupon'),

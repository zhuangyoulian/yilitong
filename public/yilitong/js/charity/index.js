$(function() {
    ...
    function isSelected() {
        ...
        var c = $('input[name="gethelp"]:checked').val();
        console.log(c);
        if (c == 4) {
            $('.kuang').addClass('hidden');
            $('.select_hostical').addClass('hidden');
        } else if (c == 5) {
            $('.kuang').removeClass('hidden');
            $('.select_hostical').addClass('hidden');
        } else if (c == 6) {
            $('.kuang').addClass('hidden');
            $('.select_hostical').removeClass('hidden');
        }

    }
    $('input[name="gethelp"]').change(function() {
        var c = $('input[name="gethelp"]:checked').val();
        console.log(c);
        if (c == 4) {
            $('.kuang').addClass('hidden');
    ...
    $('#submitJuan').click(function() {
        ...
        var value = {
            ...
            is_donate: $("input[name^='materialUse']:checked").val(),
    ...
    $('#submitDonation').click(function() {
        ...
        var value = {
            ...
            comment: $("#comment").val(),
            materials: $("input[name='materials']").val(),
            beneficiaries: $('input[name^="hostical"]:checked').val(),
        }
        console.log(value);
        if (value.is_donate != '' && value.materials != '' && value.budget != '' && value.do_name != '' && value.do_phone != '') {
            if (value.is_purchase == 1 && (((value.is_donate == 2 || value.is_donate == 3) && value.beneficiaries != '') || value.is_donate == 1)) {
        ...
        var juanz_username = $('#juanz_username').val().trim();
        var juanz_tel = $('#juanz_tel').val().trim();
        var yuansuan = $('#yuansuan').val().trim();
        var checked_recipient = [];
        $('input[name^="recipient"]:checked').each(v => {
            checked_recipient.push(v);
        });
        if (juanz_username == '') {
            $('#submitDonation .error').html('请输入联系人姓名');
        } else if (juanz_tel == "") {
            $('#submitDonation .error').html('请输入联系方式');
        } else if (yuansuan == "") {
            $('#submitDonation .error').html('请输入预算');
        } else if (checked_recipient.length == 0) {
            $('#submitDonation .error').html('请选择物资');
            // } else {
            //     $('.wrap').removeClass('hidden');
            //     $('.successBox').removeClass('hidden');
        }

...
class Agen extends Base{
     ...
     */
    public function a_gen_list(){
        $GoodsLogic = new GoodsLogic();
        $categoryList = $GoodsLogic->agen_getSortCategory();
    ...
    public function ajaxGoodsList(){
        ...
        //         Db::name('agen_goods')->where('goods_id',$value['goods_id'])->update(array('is_on_sale' => 0 , 'is_hot' => 0 , 'is_new' => 0 ));
        //     }
        // }

        $catList = D('agen_goods_category')->select();
    ...
    public function addEditGoods()
    {
        ...
        $GoodsLogic = new GoodsLogic();
        $Goods = new \ylt\admin\model\AgenGoods(); //
        ...
        if ((I('is_ajax') == 1) && IS_POST) {
           ...
           // $GoodsLogic->saveGoodsAttr($goods_id,I('goods_type')); // 处理商品 属性
            ...
            $return_arr = array(
                'status' => 1,
                'msg' => '操作成功',
                'data' => array('url' => Url::build('admin/Agen/a_gen_list')),
            );
            $this->ajaxReturn($return_arr);
        }

        $goodsInfo = Db::name('agen_goods')->where('goods_id=' . I('GET.id', 0))->find();
        $level_cat = $GoodsLogic->agen_find_parent_cat($goodsInfo['cat_id']); // 获取分类默认选中的下拉框
        $cat_list = Db::name('agen_goods_category')->where("parent_id = 0 and is_show = 1 ")->select(); // 已经改商品成联动菜单
        $goodsType = $GoodsLogic->agen_goodsType();
        ...
        $goodsImages = Db::name("agen_goodsImages")->where('goods_id =' . I('GET.id', 0))->select();
    ...
    public function delGoods()
    {
        ...
        if (!empty($goods_id && is_array($goods_id))) {
            ...
            // 判断此商品是否有订单
            $c1 = Db::name('AgenOrderGoods')->alias('g')->join('order o','o.order_id = g.order_id')->where("g.goods_id = $value")->where("o.confirm_time > $time")->field('goods_id')->find();
            $c1 && $error .= "商品ID：".$c1['goods_id']." 有近一年的订单记录,不得删除! <br/>";
            

             // 商品退货记录
            $c1 = Db::name('agen_back_order')->alias('g')->join('order o','o.order_id = g.order_id')->where("g.goods_id = $value")->where("o.confirm_time > $time")->field('goods_id')->find();
            ...
            $goods_content=Db::name("AgenGoods")->where('goods_id ='.$value)->field('goods_content')->value('goods_content');
            if ($goods_content) {
                $v = $this->remove_content_img($goods_content);
            }
            $image_url=Db::name("agen_goods_images")->where('goods_id ='.$value)->field('image_url')->select();
            if ($image_url) {
                foreach ($image_url as $ke => $valu) {
                    @unlink ('.'.$valu['image_url']);       //删除图片文件
                    Db::name("agen_goods_images")->where('goods_id ='.$value)->delete();  //删除商品相册数据库内容
                }
            }

            $src=Db::name("agen_spec_image")->where('goods_id ='.$value)->field('src')->select();
            if ($src) {
                foreach ($src as $k => $val) {
                    @unlink ('.'.$val['src']);              //删除图片文件
                    Db::name("agen_spec_image")->where('goods_id ='.$value)->delete();  //删除商品规格图片数据库内容
                }
            }
            //删除此商品相关内容      
            Db::name("AgenGoods")->where('goods_id ='.$value)->delete();  //商品表
            Db::name("agen_cart")->where('goods_id ='.$value)->delete();  // 购物车
            Db::name("agen_goods_price")->where('goods_id ='.$value)->delete();  //商品规格
    ...
    public function RefreshGoods()
    {
        ...
        $model = Db::name('AgenGoods');
        Db::name('AgenGoods')->where($where)->update(array('last_update' => time()));
        $this->success('一键刷新成功',Url::build('Admin/Agen/a_gen_list'));
    }
    ...
    public function categoryList(){
        $GoodsLogic = new GoodsLogic();
        $cat_list = $GoodsLogic->agen_goods_cat_list();
        $this->assign('cat_list',$cat_list);
        return $this->fetch();
    } 
    ...
    public function addEditCategory(){
        ...
        if(IS_GET)
        {
            $goods_category_info = Db::name('agen_goodsCategory')->where('id='.I('GET.id',0))->find();
            $level_cat = $GoodsLogic->agen_find_parent_cat($goods_category_info['id']); // 获取分类默认选中的下拉框
            $cat_list = Db::name('agen_goods_category')->where("parent_id = 0")->select(); // 已经改成联动菜单
            $this->assign('level_cat',$level_cat);
            $this->assign('cat_list',$cat_list);
        ...
        $GoodsCategory =  Db::name('agen_goods_category'); //
        ...
        {	
            ...
            if ($type == 2)
            {
                ...
                $GoodsLogic->agen_refresh_cat(I('id'));
            ...
            else
            {
                ...
                $GoodsLogic->agen_refresh_cat($insert_id);
				adminLog('添加分类 '.input('name').'');
            }
            $return_arr = array(
                'status' => 1,
                'msg'   => '操作成功',
                'data'  => array('url'=>Url::build('Admin/Agen/categoryList')),
            );
            $this->ajaxReturn($return_arr);
    ...
    public function delGoodsCategory(){
        ...
        $count > 0 && $this->error('该分类下还有分类不得删除!',Url::build('Admin/Agen/categoryList'));
        // 判断是否存在商品
        $goods_count = Db::name('agen_goods')->where("cat_id = {$id}")->count('1');
        $goods_count > 0 && $this->error('该分类下有商品不得删除!',Url::build('Admin/Agen/categoryList'));
        // 删除分类
        DB::name('agen_goods_category')->where('id',$id)->delete();
        $this->success("操作成功!!!",Url::build('Admin/Agen/categoryList'));
    }

    /**
     * [goodsTypeList 商品类型  用于设置商品的属性]
     * @return [type] [description]
     */
    public function goodsTypeList(){
        $model = Db::name("AgenGoodsType");
		$keyword = I('keyword');
    ...
    public function addEditGoodsType()
    {
        ...
        if (IS_POST) {
            ...
            if ($id){
				 DB::name('AgenGoodsType')->update($data);
			} 
            else{
				if(DB::name('AgenGoodsType')->where('name',$data['name'])->find())
					$this->success("模型已经存在，请勿重复添加!", Url::build('Admin/Agen/goodsTypeList'));
				else
				 DB::name('AgenGoodsType')->insert($data);
			}
            $this->success("操作成功!!!", Url::build('Admin/Agen/goodsTypeList'));
            exit;
        }
    ...
    public function delGoodsType()
    {
        // 判断 商品规格
        $id = $this->request->param('id');
        $count = Db::name("AgenSpec")->where("type_id = {$id}")->count("1");
        $count > 0 && $this->error('该类型下有商品规格不得删除!',Url::build('Admin/Agen/goodsTypeList'));
        // 删除分类
        Db::name('AgenGoodsType')->where("id = {$id}")->delete();
        $this->success("操作成功!!!",Url::build('Admin/Agen/goodsTypeList'));
    } 
    ...
    public function specList(){       
        $goodsTypeList = Db::name("AgenGoodsType")->select();
        $this->assign('goodsTypeList',$goodsTypeList);
        return $this->fetch();
    }
    ...
    public function ajaxSpecList(){ 
        ...
        $this->assign('page',$show);// 赋值分页输出
        $goodsTypeList = Db::name("AgenGoodsType")->select(); // 规格分类
    ...
    public  function addEditSpec(){
    	$AgenSpec = new \ylt\admin\model\AgenSpec(); 
        $type = I('id') > 0 ? 2 : 1; // 标识自动验证时的 场景 1 表示插入 2 表示更新
        if((I('is_ajax') == 1) && IS_POST)//ajax提交验证
        {        
            $post_data = I('post.');
            if ($type == 2) {
                Db::name('agen_spec')->update($post_data); // 写入数据到数据库
                $AgenSpec->afterSave(I('id'));
            } else {
				$count = Db::name('agen_spec')->where('type_id',I('type_id'))->count();
				if($count > 1 ){
					$return_arr = array(
					'status' => -1,
					'msg' => '同一模型不允许添加超过2种规格分类！',
					'data' => array('url' => Url::build('/Admin/Agen/specList')),
				);
				$this->ajaxReturn($return_arr);
				}
                $insert_id = Db::name('agen_spec')->insertgetID($post_data);
                $AgenSpec->afterSave($insert_id);
            }
            $return_arr = array(
                'status' => 1,
                'msg' => '操作成功',
       ...
       // 点击过来编辑时                 
       $id = I('id/d',0);
       $spec = Db::name('agen_spec')->find($id);
       $GoodsLogic = new GoodsLogic();  
       $items = $GoodsLogic->agen_getSpecItem($id);
    ...
    public function addSpecItem(){  
		...
		if(Db::name('agen_spec_item')->where($data)->find()){
			$add_id = 0;
		}else{
			Db::name('agen_spec_item')->insert($data);
			$add_id =  Db::name('agen_spec')->where('id',$spec_id)->value('type_id');
		}
    ...
    public function delGoodsSpec()
    {
        $id = I('id');
        // 判断 商品规格项
        $count = Db::name("AgenSpecItem")->where("spec_id = {$id}")->count("1");
        // 删除分类
        Db::name('AgenSpec')->where("id = {$id}")->delete();
        $this->redirect('Admin/Agen/specList');
    } 
    ...
    public function ajaxGetSpecSelect(){
        ...
        //查询商品规格
        $specList = Db::name('AgenSpec')->where("type_id = ".I('get.spec_type/d'))->order('`order` desc')->select();
        foreach($specList as $k => $v)
            $specList[$k]['spec_item'] = Db::name('AgenSpecItem')->where(array('spec_id' => $v['id']))->order('id')->column('id,item'); // 查询商品规格的参数获取规格项

        $items_id = Db::name('AgenGoodsPrice')->where('goods_id = '.$goods_id)->getField("GROUP_CONCAT(`key` SEPARATOR '_') AS items_id");//查询商品价格
        $items_ids = explode('_', $items_id);

        // 获取商品规格图片
        if($goods_id)
        {
            $specImageList = Db::name('AgenSpecImage')->where("goods_id = $goods_id")->column('spec_image_id,src');
        }
    ...
    public function ajaxGetSpecInput(){     
         ...
         $str = $GoodsLogic->agen_getSpecInput($goods_id ,I('post.spec_arr/a',[[]]));
         exit($str);   
    }
    ...
    public function ajaxOrderIndex(){
        ...
        $count = Db::name('agen_order')->where($condition)->count();
        ...
        $show = $Page->show();
        //获取订单列表
        $orderList = $orderLogic->agen_getOrderList($condition,$sort_order,$Page->firstRow,$Page->listRows);
        $orderList_s = Db::name('agen_order')->where($condition)->order('order_id')->field('order_amount')->select();
    ...
    public function orderDetail($order_id){
        $orderLogic = new OrderLogic();
        $order = $orderLogic->agen_getOrderInfo($order_id);
        $orderGoods = $orderLogic->agen_getOrderGoods($order_id);
        $button = $orderLogic->agen_getOrderButton($order);
        // 获取操作记录
        $action_log = Db::name('agen_order_action')->where(array('order_id'=>$order_id))->order('log_time desc')->select();
        $userIds = array();
        //查找用户昵称
        ...
        //发货单
        $shipping = Db::name('agen_shipping_order')->where('order_id',$order_id)->select();
    ...
    public function update_pr(){
        $order_id = I('order_id');
        $shipping = Db::name('agen_order')->where('order_id',$order_id)->update(['order_status'=>1]);
        if ($shipping) {
            $this->success('修改成功');
        }
    }
    ...
    public function update_pa(){
        $order_id = I('order_id');
        $shipping = Db::name('agen_order')->where('order_id',$order_id)->update(['pay_status'=>1]);
        if ($shipping) {
            $this->success('修改成功');
        }
    }
    ...
    public function order_action(){
        ...
        if($action && $order_id){
            ...
            $a = $orderLogic->agen_orderProcessHandle($order_id,$action,array('note'=>I('note'),'admin_id'=>0));
            if($a !== false){
                if ($action == 'remove') {
                    exit(json_encode(array('status' => 1, 'msg' => '操作成功', 'data' => array('url' => Url::build('admin/Agen/orderIndex')))));
                }
                exit(json_encode(array('status' => 1,'msg' => '操作成功')));
            }else{
                if ($action == 'remove') {
                    exit(json_encode(array('status' => 0, 'msg' => '操作失败', 'data' => array('url' => Url::build('admin/Agen/orderIndex')))));
                }
                exit(json_encode(array('status' => 0,'msg' => '操作失败')));
            }
        }else{
            $this->error('参数错误',Url::build('Admin/Agen/orderDetail',array('order_id'=>$order_id)));
        }
    }
    /**
     * [delivery_info 去发货]
    ...
    public function delivery_info(){
        ...
        $order = $orderLogic->agen_getOrderInfo($order_id);
        $orderGoods = $orderLogic->agen_getOrderGoods($order_id);
        $delivery_record = Db::name('agen_shipping_order')->alias('d')->join('__ADMIN_USER__ a','a.admin_id = d.admin_id')->where('d.order_id='.$order_id)->select();
        if($delivery_record){
    ...
    public function deliveryHandle(){
        $orderLogic = new OrderLogic();
        $data = I('post.');
        $res = $orderLogic->agen_deliveryHandle($data);
    ...
    public function editprice($order_id){
        ...
        if(IS_POST){
            ...
            $row = Db::name('agen_order')->where(array('order_id'=>$order_id))->update($update);
            if(!$row){
                $this->success('没有更新数据',Url::build('Admin/Agen/editprice',array('order_id'=>$order_id)));
            }else{
                // $email = Db::name('agen_user')->where(array('admin_id'=>$order['user_id']))->value('email');
                // // //修改价格后提醒客户
                // if ($email) {
                //     send_email($email,'一创科技订单价格修改成功','您好！一创科技订单价格已修改成功，请及时处理。');
                // }
                adminLog("修改订单价格：应付金额改为：".$update['order_amount']."");
                $this->success('操作成功',Url::build('Admin/Agen/orderDetail',array('order_id'=>$order_id)));
            }
            exit;
        }
        $this->assign('order',$order);
     ...
     public function export_order()
    {
        ...
        $sql = "select *,FROM_UNIXTIME(add_time,'%Y-%m-%d') as create_time from __PREFIX__agen_order $where order by order_id"; //echo $sql;exit;
    ...
    public function ajaxSuggestlist(){
        ...
        $count = Db::name('agen_comment')->where($where)->count();
        $Page = $pager = new AjaxPage($count,15);
        $show = $Page->show();
                
        $comment_list = Db::name('agen_comment')->where($where)->order('add_time DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function suggesDetail(){
        $id = I('get.id/d');
        $res = Db::name('agen_comment')->where(array('comment_id'=>$id))->find();
        if ($res['txt']) {
            $res['txt'] = "/public/upload/yichuang/".$res['txt'];
        }
        if ($res['images']) {
            $image = explode(',',$res['images']);
            foreach ($image as $key => $value) {
                $images[] = "/public/upload/yichuang/".$value;
            }
        }
        if(!$res){
    ...
    public function suggesDel(){
        $id = I('get.id/d');
        $row = Db::name('agen_comment')->where(array('comment_id'=>$id))->delete();
        if($row){
            $this->success('删除成功');
        }else{
            $this->error('删除失败');
    ...
    public function userList(){
        // Db::execute('TRUNCATE table ylt_agen_log');  //清空表
        $list = array();
        $keywords = I('keywords/s');
        if(empty($keywords)){
            $res = Db::name('agen_user')->order('admin_id')->select();
        }else{
            $res = DB::name('agen_user')->where('user_name','like','%'.$keywords.'%')->order('admin_id')->select();
        }
        if($res){
            foreach ($res as $val){
    ...
    public function user_info(){
        ...
        if($admin_id){
            $info = Db::name('agen_user')->where("admin_id", $admin_id)->find();
            $info['password'] =  "";
        ...
        $this->assign('act',$act);
        $role = Db::name('agen_user')->where('1=1')->select();
    ...
    public function adminHandle(){
        ...
        if($data['act'] == 'add'){
            ...
            unset($data['admin_id']);           
            $data['add_time'] = time();
            if(Db::name('agen_user')->where("user_name", $data['user_name'])->count()){
                $this->error("此用户名已被注册，请更换",Url::build('Admin/Agen/user_info'));
            }else{
                $r = Db::name('agen_user')->insert($data);
                adminLog('添加用户'.$data['user_name'].'');
                agen_adminLog('添加用户'.$data['user_name'].'');
            }
        }
        ...
        if($data['act'] == 'edit'){
            ...
            $r = Db::name('agen_user')->where('admin_id', $data['admin_id'])->save($data);
            adminLog('修改用户信息'.$data['user_name'].'');
            agen_adminLog('修改用户信息'.$data['user_name'].'');
        }
        if($data['act'] == 'del' && $data['admin_id']>1){
            $r = Db::name('agen_user')->where('admin_id', $data['admin_id'])->delete();
            adminLog('删除用户 '.$data['admin_id'].'');
            agen_adminLog('删除用户 '.$data['admin_id'].'');
            exit(json_encode(1));
        }
        
        if($r){
            $this->success("操作成功",Url::build('Admin/Agen/userList'));
        }else{
            $this->error("操作失败",Url::build('Admin/Agen/userList'));
        }
    }
    ...
    public function r_password(){
        ...
        $update = Db::name('agen_user')->where('admin_id', $admin_id)->update($data);
        if ($update) {
    ...
    public function operation(){
        $p = I('p/d',1);
        $logs = DB::name('agen_log')->alias('l')->join('agen_user a','a.admin_id =l.admin_id')->order('log_time DESC')->page($p.',20')->select();
        $this->assign('list',$logs);
        $count = DB::name('agen_log')->where('1=1')->count();

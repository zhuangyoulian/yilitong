...
class RedGift extends Base{
    ...
    public function addEditGoods()
    {
        ...
        if ((I('is_ajax') == 1) && IS_POST) {
            ...
            if ($type == 2) {
                $goods_id = I('goods_id');
                if ($data['goods_group']) {
                    $Goods->goods_group = implode(',',$data['goods_group']);
                }
                $Goods->is_on_sale = 0;
               ...
               if(file_exists("./runtime/html/home_goods_goodsinfo_{$goods_id}.html")){
                ...
                $Goods->supplier_id = 686; // 入驻商ID
                $Goods->red_supplier_id = 686; // 入驻商ID
                $Goods->supplier_name = '红礼供应链';
                $Goods->examine = 1;
                $Goods->is_on_sale = 0;
                $Goods->save(); // 写入数据到数据库
        ...
        if ($goodsInfo['goods_group']) {
            $prom_goods = Db::name('red_goods')->field('goods_id,goods_name,shop_price,store_count')->where("goods_id in ($goodsInfo[goods_group])")->select();
        ...
        // $goodsImages = Db::name("goodsImages")->where('red_goods_id =' . I('GET.id', 0))->select();
    ...
    *礼包添加商品
    ...
    {
        $goods_id = I('goods_id');
        $where = ' is_delete = 0 and examine = 1 and is_group != 1';//搜索条件
        if (!empty($goods_id)) {
        ...
        $count = Db::name('red_goods')->where($where)->where(' is_delete = 0 and examine = 1')->count();
        $Page = new Page($count, 10);
        $goodsList = Db::name('red_goods')->where($where)->where(' is_delete = 0 and examine = 1')->order('goods_id DESC')->limit($Page->firstRow . ',' . $Page->listRows)->select();
        ...
        if (!empty($goods_id && is_array($goods_id))) {
            ...
            if ($image_url) {
                foreach ($image_url as $ke => $valu) {
                    ...
                    if (Db::name("goods_images")->where('red_goods_id ='.$value)->find()) {
                        Db::name("goods_images")->where('red_goods_id ='.$value)->delete();  //删除商品相册数据库内容
                    }
                }
            }
            ...
            // 删除此商品        
            Db::name("RedGoods")->where('goods_id ='.$value)->delete();          //红礼商品表
            Db::name("red_goods_images")->where('goods_id ='.$value)->delete();  //红礼商品相册
            Db::name("red_goods_price")->where('goods_id ='.$value)->delete();   //红礼商品规格
            Db::name("red_spec_image")->where('goods_id ='.$value)->delete();    //红礼商品规格图片
            Db::name("red_cart")->where('goods_id ='.$value)->delete();          //红礼购物车
    ...
    {
        $where = 'is_delete = 0 and ( examine = 1 or is_on_sale =1 ) '; // 搜索条件
        Db::name('RedGoods')->where($where)->update(array('last_update' => time()));
        Db::name('Goods')->where($where)->where('supplier_id',686)->update(array('last_update' => time()));
  ...
  //       $goods_counts = Db::name('goods')->where("cat_id = {$id}")->count('1');
  //       ($goods_count or $goods_counts > 0)&& $this->error('该分类下有商品不得删除!',Url::build('Admin/RedGift/red_categoryList'));
  //       // 删除分类
  ...
  //               Db::name('spec')->update($post_data); // 写入数据到数据库
  ...
  //   }
    ...
    public function _initialize() {
        ...
        $this->red_order_status = config('AGEN_ORDER_STATUS');
        ...
        $this->assign('red_order_status',$this->red_order_status);
    ...
    public function orderDetail($order_id){
        ...
        $orderGoods = $orderLogic->red_getOrderGoods($order_id); //红礼
        $orderGoods_yi = $orderLogic->getOrderGoods($order_id);  //一礼通
        $button = $orderLogic->agen_getOrderButton($order);      //一创和红礼共用按钮,红礼商城的订单使用
        $button_yi = $orderLogic->getOrderButton($order);        //一礼通来源订单使用
        foreach ($orderGoods as $key => $value) {       //查询是否为红礼组合礼包商品
            if ($value['is_group'] !=0 and !empty($value['goods_group'])) {
                ...
                foreach ($red_goods as $ke => $val) {
                    $val['is_group'] = 1;
                    $orderGoods[] = $val;
                }
            }
        }
        foreach ($orderGoods_yi as $key => $value) {    //查询是否为红礼组合礼包商品
            if ($value['is_group'] !=0 and !empty($value['goods_group'])) {
                $red_goods = Db::name('red_goods')->where("goods_id in ($value[goods_group])")->field('goods_id,goods_name,shop_price,goods_thumb,red_supplier_id')->select();
                foreach ($red_goods as $key => $value) {
                    $value['is_group'] = 1;
                    if (Db::name('goods')->where('red_goods_id', $value['goods_id'])->value('goods_id')) {
                        $value['goods_id'] = Db::name('goods')->where('red_goods_id', $value['goods_id'])->value('goods_id');
                    }else{
                        $value['is_goods_id'] = 2;
                    }
                    $orderGoods_yi[] = $value;
        ...
        $this->assign('orderGoods_yi',$orderGoods_yi);
        ...
        $this->assign('button_yi',$button_yi);
        return $this->fetch();
    }
    ...
    public function delivery_info(){
        ...
        $order = $orderLogic->red_getOrderInfo($order_id);
        $orderGoods = $orderLogic->red_getOrderGoods($order_id);
        $orderGoods_yi = $orderLogic->getOrderGoods($order_id);

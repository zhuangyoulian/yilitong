...
class Coupon extends Base {
    ...
    public function index(){
        ...
        // dump($supplier_id);die;
        $count =  Db::name('coupon')->where('supplier_id',$supplier_id)->count();
        $Page = new Page($count,10);
        $show = $Page->show();
        $lists_ = Db::name('coupon')->where('supplier_id',$supplier_id)->order('add_time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function coupon_info(){
        ...
        if($cid){
            ...
            if ($coupon['goods_id']) {
                $prom_goods = Db::name('goods')->where("goods_id in ($coupon[goods_id])")->select();
        ...
        if(IS_POST){
            ...
            $where=explode(',',$data['goods_id']);
            $couponValidate = \think\Loader::validate('Coupon','validate','','admin');
             // dump($couponValidate);
             // die;
            if (!$couponValidate->batch()->check($data)) {
                $this->ajaxReturn(['status' => 0, 'msg' => '操作失败', 'result' => $couponValidate->getError()]);
            }
            if(empty($data['id'])){   //无ID为添加
                $data['add_time'] = time();
                $row = Db::name('coupon')->insert($data);
                $good= Db::name('coupon')->field('id')->order('id','desc')->find();
                ...
                if($createnum < $data['createnum']){
                    $this->ajaxReturn(['status' => -11, 'msg' => '发放数量不可减少', 'result' => '']);
                }
                $row = Db::name('coupon')->where(array('id'=>$data['id']))->update($data);
                $goods['prom_id']=$data['id'];
                // 使用 REPLACE 方法替换/删除字段内的某个字符串
                // Db::query("update ylt_goods set prom_type=REPLACE(prom_type,',4','') where prom_id REGEXP ',$cid'");
                // REPLACE函数会替换相同的所有字符串，用以下代码限制只替换一次字符串
                ...
                Db::query("update ylt_goods set prom_id=REPLACE(prom_id,',$cid','') where prom_id REGEXP ',$cid'");
                Db::query("update ylt_supplier set coupon_su=REPLACE(coupon_su,',$goods[prom_id]','') where  coupon_su REGEXP ',$goods[prom_id]'");
            }
            if($row !== false){
                if ($data['goods_id']) {
                    ...
                    Db::query("update ylt_goods set prom_type=CONCAT(prom_type,',4') where goods_id='$where[$i]'");
                    }
                }
    ...
    */
    public function make_coupon(){
        ...
        $this->success("发放成功",Url::build('Supplier/Coupon/index'));
        exit;
    ...
    public function del_coupon(){
        ...
        Db::query("update ylt_goods set prom_type =concat(SUBSTRING(prom_type ,1,position(',4' in prom_type )-1),'',substring(prom_type ,position(',4' in prom_type )+length(',4'))) where prom_id like '%,$cid%'");
        Db::query("update ylt_goods set prom_id=REPLACE(prom_id,',$cid','') where prom_id REGEXP ',$cid'");
        Db::query("update ylt_supplier set coupon_su=REPLACE(coupon_su,',$cid','') where coupon_su REGEXP ',$cid'");
        if($row){
            //删除此类型下的优惠券

...
class Goods extends Base {
    ...
    public  function addBrand(){
        ...
        if(IS_POST)
        {
            ...
            $this->success("操作成功!!!",Url::build('Supplier/Goods/brandList',array('p'=>I('p'))));
            exit;
        }
    ...
    public function addEditGoods()
    {
		$supplier_id = session('supplier_id'); 
        $GoodsLogic = new GoodsLogic();
        $Goods = new \ylt\supplier\model\Goods(); //
        ...
        if ((I('is_ajax') == 1) && IS_POST) {
            ...
            if(!$validate->batch()->check(I('post.')))
            {
                ...
                if ($type == 2) {
                    ...
                    // 修改商品后购物车的商品价格也修改一下
                    Db::name('cart')->where("goods_id = $goods_id and spec_key = ''")->update(array(

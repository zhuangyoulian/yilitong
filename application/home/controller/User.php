...
class User extends Base{
    ...
    */
    ...
    public function order_list(){
        ...
        foreach($order_list as $k=>$v)
        {
            ...
            //是否有退换货的进度
            $order_list[$k]['back_order_id'] = Db::name('back_order')->where(["user_id"=>$this->user_id,'order_id'=>$v['order_id']])->value('id');
        }


        $rs=array('status'=>'1','info'=>'请求成功','order_status'=>config('ORDER_STATUS'),'shipping_status'=>config('SHIPPING_STATUS'),'pay_status'=>config('PAY_STATUS'),'active_status'=>I('get.type'),'active'=>'order_list','lists'=>$order_list,'page'=>$show,'count'=>$count);
        exit(json_encode($rs));
    }
    ...
    public function back_order()
    {
        ...
        {
            exit(json_encode(array('status'=>-111,'msg'=>'','id'=>$return_order['id'])));
        } 
    ...
    {        
        ...
        $rs=array('status'=>'1','info'=>'请求成功','goodsList'=>$goodsList,'list'=>$list,'page'=>$page->show(),'active'=>'back_goods_list');
        exit(json_encode($rs));
    }
    ...
    public function back_goods_info()
    {
        ...
        $rs=array('status'=>'1','info'=>'请求成功','goods'=>$goods,'msg'=>$msg,'return_goods'=>$return_goods,'return_order'=>$return_order);
        exit(json_encode($rs));
    }
 ...
 //    }
    ...
    public function comment(){
        ...
        $rs=array('status'=>'1','info'=>'请求成功','page'=>$data['show'],'comment_list'=>$data['result'],'active'=>'comment');
        exit(json_encode($rs));
    }
